...
[![Version](https://img.shields.io/badge/version-1.3.0-2ea44f?style=flat-square)](geolibre-inaturalist-extractor/plugin.json)
...
[![Min GeoLibre](https://img.shields.io/badge/GeoLibre-%E2%89%A5%200.9.0-0b7285?style=flat-square)](https://github.com/opengeos/GeoLibre)
...
- **Plugin id:** `geolibre-inaturalist-extractor` · **Version:** 1.3.0
...
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------- |
| Toolbar and panel     | A toolbar button (vector magnifying-glass + paw icon) opens a right-sidebar panel.                                                                                                                                                                                                                                                 | `registerRightPanel`               |
...
| Max observations      | 10 to 2000, paged 200 at a time against the public API, following iNaturalist's guidance to request small-to-medium batches instead of bulk-scraping.                                                                                                                                                                              | Paging of 200                      |
...
| Click popup           | Click a point to see species, observer, date, quality grade and iconic taxon. Up to three photo thumbnails are shown (click one for the full-size photo); with no photo, only the text details appear. The DOM popup is positioned with `map.project()` and kept in sync on `move`, so no second copy of `maplibre-gl` is bundled. | `queryRenderedFeatures`, `project` |
