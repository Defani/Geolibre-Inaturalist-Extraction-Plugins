...
- GeoLibre 3.0.0 or newer. The map-view and layer-extent APIs the plugin relies on
  are not all available on older versions, so the marketplace entry blocks them.
...
zip ../geolibre-inaturalist-extractor-1.3.1.zip plugin.json index.js style.css
```
...
```powershell
cd geolibre-inaturalist-extractor
Compress-Archive -Path plugin.json, index.js, style.css -DestinationPath ..\geolibre-inaturalist-extractor-1.3.1.zip
```
